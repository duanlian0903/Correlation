...
def convert_comma_string_into_number(comma_string):  # tested
    try:
        ...
        aocm.show_exception_message('Having error converting string [' + str(comma_string) + '] to number, and return nan value.')
...
def convert_special_letter_ending_string_into_number(special_letter_ending_string):  # tested
    try:
        if special_letter_ending_string.endswith('%'):
            ...
            aocm.show_exception_message('Having unexpected ending [' + str(special_letter_ending_string) + '], and return nan value.')
            result = get_math_nan_value()
    except:
        aocm.show_exception_message('Having error converting string [' + str(special_letter_ending_string) + '] to number, and return nan value.')
        result = get_math_nan_value()
    return result
...
def get_original_percent_change_number(current_number, base_number):  # tested
    percent_change = get_math_nan_value()
    try:
        if (current_number > 0) & (base_number > 0):
            percent_change = (current_number - base_number)/base_number
    except:
        aocm.show_exception_message('Having unexpected error[' + str(current_number) + ', ' + str(base_number) + '], and return nan value.')
    return percent_change


def get_transformed_percent_change_number(current_number, base_number):  # tested
    percent_change = get_math_nan_value()
    try:
        if (current_number > 0) & (base_number > 0):
            if current_number > base_number:
                ...
                percent_change = -(base_number - current_number)/current_number
    except:
        aocm.show_exception_message('Having unexpected error[' + str(current_number) + ', ' + str(base_number) + '], and return nan value.')
    return percent_change
...
def get_percent_position(current_number, highest_number, lowest_number):  # tested
    ...
    try:
        ...
        aocm.show_exception_message('Having unexpected error[' + str(current_number) + ', ' + str(highest_number) + ', ' + str(lowest_number) + '], and return nan value.')
...
def get_relative_percent_portion(portion1_number, portion2_number):  # tested
    ...
    try:
        ...
        aocm.show_exception_message('Having unexpected error[' + str(portion1_number) + ', ' + str(portion2_number) + '], and return nan value.')
    return portion
